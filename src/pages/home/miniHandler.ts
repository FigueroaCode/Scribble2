...
export class MiniHandler {
  ...
  mergeNotes(){
    //Iterate through the changelog.
    for( let change of this.changeLog){
      //More positive votes than negative votes. This can be rebalanced later.
      if(change.approvedVotes > change.disapprovedVotes){
        //Insertion of new sentence.
        if(change.originalSentence == "None"){
          let firstHalf = this.publicSentences.slice(0, change.indexOfChange)
          firstHalf.push(change.originalSentence);
          let secondHalf = this.publicSentences.slice(change.indexOfChange, this.publicSentences.length);
          this.publicSentences = firstHalf.concat(secondHalf);

          //Update existing sentence.
        }else{
          this.publicSentences[change.indexOfChange] = change.originalSentence;
        }
      }
    }
  }//End of Method
